...
export default function TasksIndex({ tasks, filters }: any) {
    ...
    const handleSearch = useRef(debounce((q: string) => {
        router.get("/leader/tasks", { ...filters, search: q }, { preserveState: true, replace: true });
    }, 500)).current;

    const handleFilter = (key: string, value: string) => {
        router.get("/leader/tasks", { ...filters, [key]: value }, { preserveState: true, replace: true });
    };

    function deleteTask(id: number) {
        router.delete(`/leader/tasks/${id}`, { preserveScroll: true });
    }

    // FIX: Replaced plain text colors with beautiful badge backgrounds and borders
            ...
            <div className="p-6 space-y-6 w-full flex flex-col h-[calc(100vh-4rem)]">
                ...
                <div className="flex items-center justify-between shrink-0">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-1">Manage deliverables and workflows for your projects.</p>
                    </div>
                    <div className="flex items-center gap-4">
                        ...
                        <Button asChild size="lg">
                            <Link href="/leader/tasks/create"><CheckSquare className="mr-2 h-4 w-4"/> New Task</Link>
                        </Button>
                    </div>
                </div>
                                                ...
                                                <TableRow key={task.id} className="hover:bg-muted/20 transition-colors">
                                                    ...
                                                    <TableCell>
                                                        <div className="flex flex-col gap-2 items-start">
                                                            ...
                                                            {/* FIX: Now using the beautiful Badge matching the Kanban board */}
                                                    ...
                                                    <TableCell className="text-right pr-6">
                                                        <DropdownMenu>
                                                            ...
                                                            <DropdownMenuContent align="end" className="w-40">
                                                                <DropdownMenuItem asChild><Link href={`/leader/tasks/${task.id}/edit`} className="w-full cursor-pointer"><Edit className="mr-2 h-4 w-4" /> Edit Details</Link></DropdownMenuItem>
