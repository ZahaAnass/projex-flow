...
export default function SprintsIndex({ sprints, filters }: any) {
    ...
    const handleSearch = useRef(debounce((q: string) => {
        router.get("/admin/sprints", { ...filters, search: q }, { preserveState: true, replace: true });
    }, 500)).current;

    const handleStatusFilter = (status: string) => {
        router.get("/admin/sprints", { ...filters, status }, { preserveState: true, replace: true });
    };

    function deleteSprint(id: number) {
        router.delete(`/admin/sprints/${id}`, { preserveScroll: true });
    }
            ...
            <div className="p-6 space-y-6 w-full">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <Button asChild size="lg">
                        <Link href="/admin/sprints/create"><Timer className="mr-2 h-4 w-4"/> New Sprint</Link>
                    </Button>
                </div>
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end" className="w-40">
                                                        ...
                                                        <DropdownMenuItem asChild>
                                                            <Link href={`/admin/sprints/${sprint.id}/edit`} className="w-full cursor-pointer"><Edit className="mr-2 h-4 w-4" /> Edit</Link>
                                                        </DropdownMenuItem>
                                                        <DropdownMenuSeparator />
                                                        <DeleteDialog title="Delete Sprint" description={`Are you sure you want to delete "${sprint.name}"? Tasks inside will lose their sprint assignment.`} onConfirm={() => deleteSprint(sprint.id)}>
