...
export default function TasksKanban({ tasks: paginatedTasks }: { tasks: PaginatedTasks }) {
    // We use the data array from the paginated object
    const [tasks, setTasks] = useState<Task[]>(paginatedTasks.data);
    const [activeColumn, setActiveColumn] = useState<Column | null>(null);
    const [activeTask, setActiveTask] = useState<Task | null>(null);

    // Sync state when page changes (pagination click)
    useEffect(() => {
    ...
    return (
        <div className="flex flex-col h-full space-y-4">
            {/* The Board Area */}
            <div className="relative flex-1 w-full flex-col overflow-hidden bg-background/30 rounded-xl p-4 md:p-6 border border-border/40">
...
function TaskCard({ task, isOverlay, onDelete }: TaskCardProps) {
    ...
    const priorityColors = {
        ...
        medium: "bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-200 dark:border-amber-900",
        high: "bg-red-500/10 text-red-600 dark:text-red-400 border-red-200 dark:border-red-900",
    };
    ...
    return (
        <div
            ...
            {...listeners}
            className={cn(
                "group relative flex cursor-grab flex-col gap-3 overflow-hidden rounded-xl border border-border/40 bg-card/80 p-4 shadow-sm transition-all hover:shadow-md hover:border-primary/20 hover:-translate-y-0.5 active:cursor-grabbing",
